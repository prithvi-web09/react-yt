// ══════════════════════════════════════════════════════════════
  ...
  async function initAdmModal(id) {
    ...
    switch (id) {
      ...
      case 'modal-unit-deployment':  storeClear('getUnits'); storeClear('getUnits'); await renderSOSAlerts(); await renderUnitList(); break;
      ...
      case 'modal-sos-alerts':       await renderSOSAlerts(); break;
    }
  }
  ...
  var _store = {};
  window._adminIssueMessage = null;
  window._adminIssueInfo = null;
  window._adminIssueKey = null;
  window._dismissedAdminIssueKey = null;

  function escapeHtml(text) {
    return String(text || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  }

  function getAdminIssueKey(info) {
    if (!info) return null;
    if (typeof info === 'string') return 'message:' + info;
    if (info.sourceType === 'embedded_sos') {
      return 'embedded_sos:' + (info.alertId || '') + ':' + (info.text || '') + ':' + (info.location || '');
    }
    if (info.sourceType === 'police_incident') {
      return 'police_incident:' + (info.docId || '') + ':' + (info.text || '') + ':' + (info.location || '');
    }
    if (info.sourceType === 'hospital_report') {
      return 'hospital_report:' + (info.docId || '') + ':' + (info.text || '') + ':' + (info.location || '');
    }
    return (info.tag || 'incident') + ':' + (info.text || '') + ':' + (info.location || '');
  }

  function dismissAdminIssueNotification(info) {
    var issue = info || window._adminIssueInfo || window._adminIssueMessage;
    var key = getAdminIssueKey(issue);
    if (key) window._dismissedAdminIssueKey = key;
    window.clearAdminIssueNotification();
  }

  function matchesLinkedSOSRecord(record, issue) {
    if (!record || !issue) return false;
    var title = String(record.title || '');
    var desc = String(record.desc || '');
    var transcript = String(record.transcript || '');
    var location = String(record.location || record.exactLocationText || '');
    var issueText = String(issue.text || issue.issue || '');
    var issueLocation = String(issue.location || '');
    var issueTranscript = String(issue.transcript || '');
    if (issueText && (title === issueText || desc.indexOf(issueText) !== -1)) return true;
    if (issueTranscript && transcript && transcript === issueTranscript) return true;
    if (issueLocation && location && location === issueLocation && issueText && desc.indexOf(issueText) !== -1) return true;
    return false;
  }

  async function resolveLinkedSOSRecords(issue) {
    if (!issue || !window._adminSession || !window._adminSession.institutionId) return;
    var db = firebase.firestore();
    var institutionId = window._adminSession.institutionId;
    var role = window._adminSession.role;

    if (role === 'police') {
      var incidentSnap = await db.collection('incidents').where('targetInstitutionId', '==', institutionId).get();
      var matchingIncidents = incidentSnap.docs.filter(function(doc) {
        var data = doc.data() || {};
        return String(data.source || '') === 'sos'
          && String(data.status || '').toUpperCase() !== 'RESOLVED'
          && matchesLinkedSOSRecord(data, issue);
      });
      for (var i = 0; i < matchingIncidents.length; i++) {
        await matchingIncidents[i].ref.update({ status: 'RESOLVED' });
      }
    }

    if (role === 'hospital') {
      var reportSnap = await db.collection('hospitals').doc(institutionId).collection('reports').get();
      var matchingReports = reportSnap.docs.filter(function(doc) {
        var data = doc.data() || {};
        return String(data.source || '') === 'sos'
          && String(data.status || '').toLowerCase() === 'pending'
          && matchesLinkedSOSRecord(data, issue);
      });
      for (var j = 0; j < matchingReports.length; j++) {
        await matchingReports[j].ref.update({ status: 'resolved' });
      }
    }
  }

  window.renderAdminIssueNotification = function(info) {
    var message = null;
    if (!info) {
      window._adminIssueMessage = null;
      window._adminIssueInfo = null;
      ...
      message = info;
      window._adminIssueMessage = info;
      window._adminIssueInfo = null;
      window._adminIssueKey = getAdminIssueKey(info);
    } else {
      var tag = info.tag || 'Incident';
      var text = info.text || 'No description';
      var location = info.location ? ' · ' + info.location : '';
      message = tag + ': ' + text + location;
      window._adminIssueMessage = message;
      window._adminIssueInfo = info;
      window._adminIssueKey = getAdminIssueKey(info);
    }
    var container = document.getElementById('adminIssueNotification');
    if (!container) return;
    if (!message) {
      container.innerHTML = '';
      return;
    }
    container.innerHTML = '<div class="admin-issue-pill" style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 14px;border-radius:12px;background:#1a1f2e;border:1px solid #334155;color:#e2e8f0;font-size:13px;">'
      + '<div style="min-width:0;">'
      + '<strong style="color:#82cfff;display:block;margin-bottom:4px;">NEW INCIDENT</strong>'
      + '<div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:calc(100vw - 300px);line-height:1.3;">' + escapeHtml(message) + '</div></div>'
      + '<button style="border:none;background:#0f172a;color:#7dd3fc;padding:6px 10px;border-radius:10px;cursor:pointer;font-size:12px;flex-shrink:0;" onclick="acknowledgeAdminIssueNotification()">Acknowledge</button>'
      + '</div>';
  };

  window.clearAdminIssueNotification = function() {
    window._adminIssueMessage = null;
    window._adminIssueInfo = null;
    window._adminIssueKey = null;
    var container = document.getElementById('adminIssueNotification');
  ...
  window.acknowledgeAdminIssueNotification = async function() {
    var issue = window._adminIssueInfo;
    dismissAdminIssueNotification(issue);
    if (!issue) return;

    try {
      await waitForFB(5000);
      if (!window._fbReady || !window._adminSession || !window._adminSession.institutionId) return;

      const db = firebase.firestore();
      const institutionId = window._adminSession.institutionId;

      if (issue.sourceType === 'embedded_sos') {
        const collection = getInstitutionCollection();
        const docRef = db.collection(collection).doc(institutionId);
        const doc = await docRef.get();
        if (!doc.exists) return;
        const alerts = doc.data().sos_alerts || [];
        if (alerts[issue.alertIndex]) {
          alerts[issue.alertIndex].status = 'acknowledged';
          await docRef.update({ sos_alerts: alerts });
        }
      } else if (issue.sourceType === 'police_incident' && issue.docId) {
        await db.collection('incidents').doc(issue.docId).update({ status: 'RESOLVED' });
      } else if (issue.sourceType === 'hospital_report' && issue.docId) {
        await db.collection('hospitals').doc(institutionId).collection('reports').doc(issue.docId).update({ status: 'resolved' });
      }
      await resolveLinkedSOSRecords(issue);
    } catch (e) {
      console.warn('acknowledgeAdminIssueNotification', e.message);
    }
  };

  async function refreshAdminIssueNotification() {
    if (!window._fbReady || !window.FB || !window._adminSession || !window._adminSession.uid) return;
    ...
    try {
      ...
      var institutionId = window._adminSession.institutionId;
      var issueInfo = null;
      var institutionDoc = await db.collection(getInstitutionCollection()).doc(institutionId).get();

      if (institutionDoc.exists) {
        var sosAlerts = (institutionDoc.data().sos_alerts || [])
          .filter(function(alert) { return (alert.status || 'pending') === 'pending'; })
          .sort(function(a, b) {
            var aTime = a.timestamp?.toDate ? a.timestamp.toDate().getTime() : 0;
            var bTime = b.timestamp?.toDate ? b.timestamp.toDate().getTime() : 0;
            return bTime - aTime;
          });

        if (sosAlerts.length) {
          var latestIndex = (institutionDoc.data().sos_alerts || []).findIndex(function(alert) {
            return alert === sosAlerts[0];
          });
          issueInfo = {
            tag: role === 'hospital' ? 'Hospital SOS' : 'Police SOS',
            text: sosAlerts[0].issue || sosAlerts[0].message || 'Incoming SOS issue',
            location: sosAlerts[0].location || 'Unknown location',
            sourceType: 'embedded_sos',
            alertIndex: latestIndex,
            alertId: sosAlerts[0].alertId || '',
            transcript: sosAlerts[0].transcript || ''
          };
        }
      }

      if (!issueInfo && role === 'police') {
        var incidentSnap = await db.collection('incidents').where('targetInstitutionId', '==', institutionId).get();
        var activeIncidents = incidentSnap.docs
          .map(function(doc) { return { id: doc.id, data: doc.data() }; })
          .filter(function(item) { return (item.data.status || '').toUpperCase() !== 'RESOLVED'; })
          .sort(function(a, b) {
            var aTime = a.data.createdAt?.toDate ? a.data.createdAt.toDate().getTime() : 0;
            var bTime = b.data.createdAt?.toDate ? b.data.createdAt.toDate().getTime() : 0;
            return bTime - aTime;
          });
        ...
        if (activeIncidents.length) {
          issueInfo = {
            tag: 'Police incident',
            text: activeIncidents[0].data.desc || activeIncidents[0].data.title || 'Active incident',
            location: activeIncidents[0].data.location || 'Unknown location',
            sourceType: 'police_incident',
            docId: activeIncidents[0].id,
            transcript: activeIncidents[0].data.transcript || ''
          };
        }
      }

      if (!issueInfo && role === 'hospital') {
        var reportSnap = await db.collection('hospitals').doc(institutionId).collection('reports').get();
        var activeReports = reportSnap.docs
          .map(function(doc) { return { id: doc.id, data: doc.data() }; })
          .filter(function(item) { return (item.data.status || '').toLowerCase() === 'pending' && item.data.source === 'sos'; })
          .sort(function(a, b) {
            var aTime = a.data.createdAt?.toDate ? a.data.createdAt.toDate().getTime() : 0;
            var bTime = b.data.createdAt?.toDate ? b.data.createdAt.toDate().getTime() : 0;
            return bTime - aTime;
        ...
        if (activeReports.length) {
          issueInfo = {
            tag: 'Hospital incident',
            text: activeReports[0].data.desc || activeReports[0].data.title || 'Active incident',
            location: activeReports[0].data.location || 'Unknown location',
            sourceType: 'hospital_report',
            docId: activeReports[0].id,
            transcript: activeReports[0].data.transcript || ''
          };
        }
      }
      ...
      var issueKey = getAdminIssueKey(issueInfo);
      if (issueKey && issueKey === window._dismissedAdminIssueKey) {
        window.clearAdminIssueNotification();
        return;
      }

      window.renderAdminIssueNotification(issueInfo);
    } catch (e) {
      console.warn('refreshAdminIssueNotification', e.message);
    }
  }
  ...
  var ACTIONS = {
    hospital:[
      { icon:'fa-triangle-exclamation', bg:'#2b1a1a', color:'#f87171', title:'SOS Alerts',       sub:'Incoming emergency alerts',    modal:'modal-sos-alerts'       },
  ...
  function showAdminDashboard(role) {
    ...
    document.getElementById('adminMain').innerHTML =
      ...
      + '<div id="adminIssueNotification" style="margin:18px 0 8px;"></div>'
      + window.getAdminActions(role)
      + '</div></div>';
    if (window._adminIssueMessage) {
      window.renderAdminIssueNotification(window._adminIssueMessage);
    }
    refreshAdminIssueNotification();
  }
  ...
  async function acknowledgeReport(id) {
    ...
    dismissAdminIssueNotification({ sourceType: 'hospital_report', docId: id });
  ...
  async function renderIncidentFeed(elId) {
    ...
    var el = document.getElementById(elId);
    if (!el) return;
    if (window._adminSession && window._adminSession.role === 'police' && window._adminSession.institutionId) {
      incs = incs.filter(function(i) {
        return !i.targetInstitutionId || i.targetInstitutionId === window._adminSession.institutionId;
      });
    }
    incs = incs.filter(i => i.status !== 'RESOLVED');
  ...
  async function resolveIncident(id) {
    dismissAdminIssueNotification({ sourceType: 'police_incident', docId: id });
  ...
  // ══════════════════════════════════════════════════════════════
  //  SOS ALERTS — Display incoming emergency alerts
  // ══════════════════════════════════════════════════════════════

  async function renderSOSAlerts() {
    const container = document.getElementById('sosAlertsList');
    if (!container) return;

    admLoading('sosAlertsList');

    try {
      await waitForFB(5000);
      
      if (!window._fbReady || !window._adminSession || !window._adminSession.institutionId) {
        admEmpty('sosAlertsList', null);
        return;
      }
      ...
      const docRef = db.collection(getInstitutionCollection()).doc(institutionId);
      const doc = await docRef.get();
      ...
      if (!alerts || alerts.length === 0) {
        if (window._adminSession?.role === 'police') {
          const incidentSnap = await db.collection('incidents').get();
          const policeIncidents = incidentSnap.docs
            .map(function(itemDoc) { return { id: itemDoc.id, data: itemDoc.data() }; })
            .filter(function(item) {
              const data = item.data || {};
              const status = (data.status || '').toUpperCase();
              return (data.source || '').toLowerCase() === 'sos'
                && status !== 'RESOLVED'
                && (!data.targetInstitutionId || data.targetInstitutionId === institutionId);
            });

          if (policeIncidents.length) {
            container.innerHTML = policeIncidents.map(function(item, idx) {
              const alert = item.data;
              const ts = alert.createdAt?.toDate ? alert.createdAt.toDate() : new Date();
              const issueText = alert.title || alert.desc || 'SOS incident';
              const exactLocation = alert.exactLocationText || alert.location || 'unknown';
              const coordsDisplay = (alert.latitude && alert.longitude)
                ? `<div style="font-size:9px;color:#666;margin-top:4px;font-family:monospace;">Coords: ${Number(alert.latitude).toFixed(6)}, ${Number(alert.longitude).toFixed(6)}</div>`
                : '';
              const mapLink = (alert.latitude && alert.longitude)
                ? `<a href="https://www.google.com/maps/?q=${alert.latitude},${alert.longitude}" target="_blank" style="font-size:10px;color:#4ade80;text-decoration:none;margin-top:6px;display:block;">Open in Maps</a>`
                : '';

              return `
                <div style="border:1px solid #222;border-radius:8px;padding:12px;margin-bottom:10px;background:#0d0d0d;">
                  <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px;">
                    <div>
                      <div style="font-weight:700;color:#fff;font-size:12px;margin-bottom:4px;">SOS ALERT #${policeIncidents.length - idx}</div>
                      <div style="font-size:10px;color:#666;">${ts.toLocaleDateString()} at ${ts.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                    </div>
                    <div style="display:flex;align-items:center;gap:4px;padding:4px 8px;background:#facc1522;border-radius:4px;border:1px solid #facc1555;">
                      <span style="font-size:10px;font-weight:600;color:#facc15;">PENDING</span>
                    </div>
                  </div>
                  <div style="font-size:11px;color:#ccc;margin-bottom:8px;line-height:1.5;padding:8px;background:#1a1a1a;border-radius:4px;">
                    <strong>Issue:</strong> ${escapeHtml(issueText)}
                  </div>
                  <div style="font-size:10px;color:#aaa;padding:8px;background:#1a1a1a;border-radius:4px;margin-bottom:8px;line-height:1.45;">
                    <strong>Victim Location:</strong> ${escapeHtml(exactLocation)}
                    ${coordsDisplay}
                  </div>
                  ${alert.distance ? `<div style="font-size:10px;color:#888;margin-bottom:6px;">Distance: ${alert.distance}</div>` : ''}
                  <div style="display:flex;gap:6px;">
                    ${mapLink}
                    <button onclick="expandFallbackSOSIncident('${item.id}')" style="padding:6px 10px;font-size:10px;background:#1e293b;border:1px solid #334155;border-radius:4px;color:#cbd5e1;cursor:pointer;font-weight:600;">Details</button>
                    <button onclick="acknowledgeFallbackSOSIncident('${item.id}')" style="flex:1;padding:6px;font-size:10px;background:#4ade8033;border:1px solid #4ade8055;border-radius:4px;color:#4ade80;cursor:pointer;font-weight:600;">Acknowledge</button>
                  </div>
                </div>
              `;
            }).join('');
      ...
      // Sort by timestamp, newest first, while preserving the original array index.
      const sorted = alerts
        .map((alert, originalIndex) => ({ alert, originalIndex }))
        .filter(({ alert }) => (alert.status || 'pending') === 'pending')
        .sort((a, b) => (b.alert.timestamp?.toDate?.() || new Date(0)) - (a.alert.timestamp?.toDate?.() || new Date(0)))
      ...
      container.innerHTML = sorted.map(({ alert, originalIndex }, idx) => {
        const ts = alert.timestamp?.toDate ? alert.timestamp.toDate() : new Date();
        const timeStr = ts.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        const dateStr = ts.toLocaleDateString();
        const statusColor = alert.status === 'pending' ? '#facc15' : alert.status === 'resolved' ? '#4ade80' : '#f87171';
        const statusIcon = alert.status === 'pending' ? '⏳' : alert.status === 'resolved' ? '✓' : '!';

        // Extract issue from message or use raw issue
        const issueText = alert.issue || alert.message?.substring(0, 120) || 'No details';
        
        // Format location with coordinates if available
        const locationDisplay = (alert.exactLocationText || alert.location)
          ? `📍 ${escapeHtml(alert.location)}`
          : '📍 Location unknown';
        
        const coordsDisplay = (alert.latitude && alert.longitude)
          ? `<div style="font-size:9px;color:#666;margin-top:4px;font-family:monospace;">Coords: ${alert.latitude.toFixed(4)}, ${alert.longitude.toFixed(4)}</div>`
          : '';
        ...
        return `
          <div style="border:1px solid #222;border-radius:8px;padding:12px;margin-bottom:10px;background:#0d0d0d;cursor:pointer;">
            <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px;">
              <div>
                <div style="font-weight:700;color:#fff;font-size:12px;margin-bottom:4px;">SOS ALERT #${alerts.length - idx}</div>
                <div style="font-size:10px;color:#666;">${dateStr} at ${timeStr}</div>
              </div>
              <div style="display:flex;align-items:center;gap:4px;padding:4px 8px;background:${statusColor}22;border-radius:4px;border:1px solid ${statusColor}55;">
                <span style="font-size:10px;font-weight:600;color:${statusColor};">${statusIcon} ${alert.status?.toUpperCase() || 'PENDING'}</span>
              </div>
            </div>
            <div style="font-size:11px;color:#ccc;margin-bottom:8px;line-height:1.5;padding:8px;background:#1a1a1a;border-radius:4px;">
              <strong>Issue:</strong> ${escapeHtml(issueText)}
            ...
            <div style="font-size:10px;color:#aaa;padding:8px;background:#1a1a1a;border-radius:4px;margin-bottom:8px;line-height:1.45;">
              <strong>Victim Location:</strong> ${escapeHtml(alert.exactLocationText || alert.location || 'unknown')}
              ${coordsDisplay}
            </div>
            ${alert.distance ? `<div style="font-size:10px;color:#888;margin-bottom:6px;">📏 Distance: ${alert.distance}</div>` : ''}
            <div style="display:flex;gap:6px;">
              ${mapLink}
              <button onclick="expandSOSAlert(${originalIndex})" style="padding:6px 10px;font-size:10px;background:#1e293b;border:1px solid #334155;border-radius:4px;color:#cbd5e1;cursor:pointer;font-weight:600;">Details</button>
              <button onclick="acknowledgeSOSAlert(${originalIndex})" style="flex:1;padding:6px;font-size:10px;background:#4ade8033;border:1px solid #4ade8055;border-radius:4px;color:#4ade80;cursor:pointer;font-weight:600;">✓ Acknowledge</button>
            </div>
          </div>
        `;
      }).join('');
      ...
      console.error('Error rendering SOS alerts:', error);
      container.innerHTML = '<div style="padding:12px;color:#f87171;font-size:12px;">Error loading alerts: ' + error.message + '</div>';
    }
  }

  function getInstitutionCollection() {
    const role = window._adminSession?.role;
    const collectionMap = {
      hospital: 'hospitals',
      police: 'police',
      shelter: 'shelters',
    ...
    return collectionMap[role] || 'hospitals';
  }

  async function expandSOSAlert(index) {
    try {
      await waitForFB(5000);
      if (!window._fbReady || !window._adminSession || !window._adminSession.institutionId) {
        admAlert('Not authenticated', 'error');
        return;
      }
      ...
      if (!doc.exists) throw new Error('Institution not found');

      const alerts = doc.data().sos_alerts || [];
      const alert = alerts[index];
      if (!alert) {
        admAlert('Alert not found', 'error');
        return;
      }

      const ts = alert.timestamp?.toDate ? alert.timestamp.toDate() : new Date();
      const when = `${ts.toLocaleDateString()} ${ts.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
      const exactLocation = alert.exactLocationText || alert.location || 'Unknown';
      const transcript = alert.transcript || alert.issue || alert.message || 'No transcript available';
      ...
      makeOverlay(
        'sosAlertDetailOv',
        'SOS ALERT DETAILS',
        '<div class="adm-field-lbl">ISSUE SUMMARY</div>'
        + '<div class="adm-field-input" style="min-height:auto;">' + escapeHtml(alert.issue || 'No summary') + '</div>'
        + '<div class="adm-field-lbl" style="margin-top:12px;">VICTIM LOCATION</div>'
        + '<div class="adm-field-input" style="min-height:auto;line-height:1.45;">' + escapeHtml(exactLocation) + '</div>'
        + '<div class="adm-field-lbl" style="margin-top:12px;">GPS COORDINATES</div>'
        + '<div class="adm-field-input" style="min-height:auto;font-family:monospace;">' + escapeHtml(coords) + '</div>'
        + '<div class="adm-field-lbl" style="margin-top:12px;">FULL TRANSCRIPT</div>'
        + '<div class="adm-field-input" style="min-height:110px;line-height:1.5;white-space:pre-wrap;">' + escapeHtml(transcript) + '</div>'
        + '<div class="adm-field-lbl" style="margin-top:12px;">RECEIVED</div>'
        + '<div class="adm-field-input" style="min-height:auto;">' + escapeHtml(when) + '</div>',
        "document.getElementById('sosAlertDetailOv')?.remove()"
      );
    } catch (error) {
      console.error('Error expanding SOS alert:', error);
      admAlert('Failed to load alert details', 'error');
    }
  }

  async function expandFallbackSOSIncident(docId) {
    try {
      const db = firebase.firestore();
      const doc = await db.collection('incidents').doc(docId).get();
      if (!doc.exists) {
        admAlert('Incident not found', 'error');
        return;
      }
      ...
      const when = `${ts.toLocaleDateString()} ${ts.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
      const exactLocation = alert.exactLocationText || alert.location || 'Unknown';
      const transcript = alert.transcript || alert.desc || alert.title || 'No transcript available';
      const coords = (alert.latitude && alert.longitude)
        ? `${Number(alert.latitude).toFixed(6)}, ${Number(alert.longitude).toFixed(6)}`
        : 'Unavailable';
      ...
      makeOverlay(
        ...
        'SOS ALERT DETAILS',
        '<div class="adm-field-lbl">ISSUE SUMMARY</div>'
        + '<div class="adm-field-input" style="min-height:auto;">' + escapeHtml(alert.title || alert.desc || 'No summary') + '</div>'
        + '<div class="adm-field-lbl" style="margin-top:12px;">VICTIM LOCATION</div>'
        + '<div class="adm-field-input" style="min-height:auto;line-height:1.45;">' + escapeHtml(exactLocation) + '</div>'
        + '<div class="adm-field-lbl" style="margin-top:12px;">GPS COORDINATES</div>'
        + '<div class="adm-field-input" style="min-height:auto;font-family:monospace;">' + escapeHtml(coords) + '</div>'
      ...
      console.error('Error expanding fallback SOS incident:', error);
      admAlert('Failed to load incident details', 'error');
    }
  }

  async function acknowledgeFallbackSOSIncident(docId) {
    try {
      dismissAdminIssueNotification({ sourceType: 'police_incident', docId: docId });
      const db = firebase.firestore();
      await db.collection('incidents').doc(docId).update({ status: 'RESOLVED' });
      admAlert('Alert acknowledged', 'success');
      await renderSOSAlerts();
    } catch (error) {
      console.error('Error acknowledging fallback SOS incident:', error);
      admAlert('Failed to acknowledge incident', 'error');
    }
  }

  async function acknowledgeSOSAlert(alertIndex) {
    try {
      await waitForFB(5000);
      ...
      if (alerts[alertIndex]) {
        ...
        dismissAdminIssueNotification({
          ...
          text: currentAlert.issue || currentAlert.message || 'Incoming SOS issue',
          location: currentAlert.location || 'Unknown location',
          transcript: currentAlert.transcript || ''
        });
        alerts[alertIndex].status = 'acknowledged';
        await docRef.update({ sos_alerts: alerts });
        await resolveLinkedSOSRecords(currentAlert);
        admAlert('Alert acknowledged', 'success');
        await renderSOSAlerts();
      }
    } catch (error) {
      console.error('Error acknowledging alert:', error);
      admAlert('Failed to acknowledge: ' + error.message, 'error');
    }
  }
